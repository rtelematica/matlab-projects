...
Z = 8;
N = 8;  %Numero de arribos, 2N será el numero de elementos en la partición

%Parametros variables
S = 5;  %Numero de servidores (3-5)
...
tit = [ 'Gráfica de ARRIBOS Y FIN, para ' num2str(N) ' arribos con u = ' num2str(u) ', lambda = ' num2str(lambda) ' y Num Servidores = ' num2str(S)];
tit2 = [ 'Gráfica de SERVIDORES OCUPADOS para ' num2str(N) ' arribos con u = ' num2str(u) ', lambda = ' num2str(lambda) ' y Num Servidores = ' num2str(S)];
...
for i = 1 : length(AF)
    ...
    if AFc(i) == 1
        ...
        AXIS([-0.2 AF(length(AF))+0.2 -3 ha+1 ])
        subplot(2,1,2), plot( [AF(f_i) AF(f_i)],[ 0 4], '--');
        AXIS([-0.2 AF(length(AF))+0.2 0 6 ])
    end
end
